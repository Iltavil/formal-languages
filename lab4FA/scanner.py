...
def readFile(inputFileString):
    ...
    constantFA = FAreader("numberFA.in")
    identifierFA = FAreader("FA.in")
    pifFile = "PIF.out"
    removeFile(pifFile)
    stFile = "ST.OUT"
    removeFile(stFile)
    with open(inputFileString) as inputFile:
        for line in inputFile:
            ...
            for eachToken in tokens:
                ...
                elif constantFA.checkWrapper(eachToken):
                    index = symbolTable.insert(eachToken)
                    genPIF("const",index,pifFile)
                elif  identifierFA.checkWrapper(eachToken):
                    index = symbolTable.insert(eachToken)
